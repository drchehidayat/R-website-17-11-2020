...
1. This presentation is intended for the beginners in R. 
2. Those who are the intermediate and advance R users are welcome to give suggestions and comments.
...
To model the Malaysians CVD risk scores as a function of age category adjusted with gender
...
- Statitical package used : *ordinal*, *tidyverse*, *haven*, and *broom*
- Ordinal logistic regression modeling was used to ascertain the relationship of age category, gender, ethnicity and geographical region to the ten-year fatal Globorisk score of Malaysians.

### Cumulative link models (Proportional Odds Models)

- Cumulative link models (CLMs) are an excellent model class for ordonal data as it allows development of flexible regression framework for details analyses.
- For this part, we will the *Cumulative Link Models(Proportional Odds Models)* which requires ordinal package.
...
globorisk <- read.csv("globoriskordinaldata.csv") 
...
# Reorder fatal.cat 
globorisk2 <- globorisk2 %>% mutate_at(vars(fatal.cat), ~fct_relevel(., c("Low Risk defined as <10%", "High Risk defined as >= 10%", "High Risk defined as >= 20%", "High Risk defined as >= 30%"
                                                                     )))
```
...
To predict the probabilities of *i-th* falls into each category for model for a new data, we need to create a new data first. We will use expand.grid() to crate a new data of class data.frame
...
newData
```
Now, we will predict the probabilities for a new data. The predicted probabilities are the probabilities that the subjects fall into each category of the outcome.
...
Looking at the results, we can see that any ethnicity at younger will fall into low CVD risk group. While, any ethnicity at older age will fall into high CVD risk group, in which Malay and Others ethnicity have comparable higher probability to have high CVD risk as compared to other ethnic counter parts. 
